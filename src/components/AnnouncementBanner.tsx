...
function isWithinActiveWindow(announcement: AnnouncementDoc, nowMs: number) {
  const startMs = announcement.startDate
    ? Date.parse(announcement.startDate)
    : NaN;
  const endMs = announcement.endDate ? Date.parse(announcement.endDate) : NaN;

  if (!Number.isNaN(startMs)) {
    const startDate = new Date(startMs);
    const now = new Date(nowMs);
    // Compare dates only (ignore time) to allow announcements scheduled for "later today" to show immediately
...
function isDismissed(id: string) {
  ...
  return window.localStorage.getItem(`announcement:dismissed:${id}`) === "1";
}

function dismiss(id: string) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(`announcement:dismissed:${id}`, "1");
}
...
export function AnnouncementBanner({
  ...
  const [announcements, setAnnouncements] = React.useState<AnnouncementDoc[]>(
    []
  );
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setAnnouncements(res.documents as unknown as AnnouncementDoc[]);
      } catch {
        if (cancelled) return;
        setAnnouncements([]);
      } finally {
  ...
  React.useEffect(() => {
    ...
    const first = announcements.find((a) => {
      if (a.isActive === false) {
        console.log("AnnouncementBanner: Skipping inactive", a.$id);
        return false;
      }
      if (!isWithinActiveWindow(a, nowMs)) {
        console.log(
          "AnnouncementBanner: Skipping outside window",
          a.$id,
          a.startDate,
          a.endDate
        );
      ...
      if (isDismissed(a.$id)) {
        console.log("AnnouncementBanner: Skipping dismissed", a.$id);
        return false;
      }
      return true;
    });
  ...
  const activeAnnouncement = React.useMemo(() => {
    ...
    return announcements.find((a) => a.$id === activeId) ?? null;
  }, [activeId, announcements]);

  const onClose = React.useCallback(() => {
    if (!activeAnnouncement) return;
    dismiss(activeAnnouncement.$id);
    const nowMs = Date.now();
    const next = announcements.find((a) => {
      if (a.$id === activeAnnouncement.$id) return false;
  ...
  }, [activeAnnouncement, announcements]);

  const onClick = React.useCallback(() => {
    if (!activeAnnouncement) return;
    const url = activeAnnouncement.buttonUrl ?? "";
    if (!url) return;
    if (url.startsWith("/")) {
  ...
  if (!activeAnnouncement) {
    if (
      ...
      return (
        <div
          className={cn(
            "mx-auto flex items-center justify-center rounded-md border border-yellow-200 bg-yellow-50 px-4 py-2 text-sm text-yellow-800 dark:border-yellow-900/50 dark:bg-yellow-900/20 dark:text-yellow-200",
            className
          )}
        >
          <span>
            Debug: No active announcements found. Check Appwrite permissions
            (Read: Any) & Collection ID.
          </span>
        </div>
      );
    }
  ...
  const title = (activeAnnouncement.title ?? "").trim();
    ...
    <div className={cn("mx-auto flex items-center justify-center", className)}>
      <AnimatePresence>
        ...
        >
          ...
          <div className="relative flex min-h-[35px] items-center gap-1 rounded-[6px] border border-[#CBE7FF] bg-[#F0F7FF] pl-2.5 pr-1 text-sm dark:border-[#003674] dark:bg-[#06193A]">
            <button
